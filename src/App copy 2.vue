...
<template>
  <main class="page">
    ...
    <template v-if="activePage === 'today'">
      ...
      <section id="today-deals" class="deals-section">
        <div class="section-head">
          <div>
            <span class="eyebrow">ANDY'S PICKS</span>
            <h2>目前值得領的優惠</h2>
          </div>
          ...
          <article v-for="deal in filteredDeals" :key="deal.id" class="deal-card">
            ...
            <div class="deal-card-body">
              ...
              <div class="deal-tags">
                ...
                <span v-if="deal.eligibleForAllUsers === false">
                  限定資格
                </span>
              </div>
    ...
    <footer class="footer">
      <div>
        <strong>SaveFlow</strong>
        <span>由 Andy 整理的日本生活省錢筆記。</span>
      </div>

      <p>
        SaveFlow 為獨立資訊整理網站，與 PayPay 及各品牌並無官方合作關係。
        優惠內容請以品牌與支付平台官方公告為準。
      </p>
    </footer>
  </main>
...
.deal-card {
  ...
  flex-direction: column;
  background: #ffffff;
  box-shadow: 0 14px 40px rgba(23, 32, 51, 0.065);
  transition:
    transform 0.2s ease,
